...
export default function PetCard({ pet, isEditing, onEdit, onCancelEdit, refreshPets }) {
  const generateAdoptionCertificate = (pet) => {
    ...
    doc.setFontSize(16);
    doc.text(`This certifies that ${pet.name}, a ${pet.species}, has been officially adopted.`, 20, 50);
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      handleAdopt(); 
      Swal.fire("Adopted!", "The pet has been successfully adopted.", "success");
    }
  });
};
...
/>
      ...
      {!pet.adopted && <button className='add-button' onClick={handleAdoptPet}>Adopt</button>}
      <button className='delete-button' onClick={handleDeletePet} style={{ color: 'white' }}>Delete</button>
      <button className='add-button' onClick={onEdit}>Edit</button>
      ...
      {pet.adoption_date && <p>Adopted On: {formatDate(pet.adoption_date)}</p>}
        </>
      )}
